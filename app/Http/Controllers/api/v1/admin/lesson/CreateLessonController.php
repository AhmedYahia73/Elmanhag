    ...
    public function create( LessonRequest $request, $ch_id ){
        ...
        foreach ($request->materials as $key => $item) {
            ...
            if ($item['source'] == 'upload') {
                if ($request->hasFile("materials.$key.material")) {
                $file = $request->file("materials.$key.material");
                $path = $file->store('uploads'); // Store file in 'storage/app/uploads'

                // You can save the $path and $material['name'] to your database here
                // Example: Material::create(['name' => $material['name'], 'file_path' => $path]);
            }
